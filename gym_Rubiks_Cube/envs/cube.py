...
class Cube:
    ...
    def printMe(self, s, chars):
        s += chars
        sys.stdout.write(chars)
        return s

    def get_state_img(self):
        image = Image.new('RGBA', (self.order * 4, self.order * 3), 'black')
        img_draw = ImageDraw.Draw(image)

        for i in range(self.order):
            for j in range(self.order, self.order * 2):
                img_draw.point((j, i), fill=Cube.colorDict[self.up[i][j - self.order]])

        for i in range(self.order, self.order * 2):
            for j in range(self.order):
                img_draw.point((j, i), fill=Cube.colorDict[self.left[i - self.order][j]])

        for i in range(self.order, self.order * 2):
            for j in range(self.order, self.order * 2):
                img_draw.point((j, i), fill=Cube.colorDict[self.front[i - self.order][j - self.order]])

        for i in range(self.order, self.order * 2):
            for j in range(self.order * 2, self.order * 3):
                img_draw.point((j, i), fill=Cube.colorDict[self.right[i - self.order][j - self.order * 2]])

        for i in range(self.order, self.order * 2):
            for j in range(self.order * 3, self.order * 4):
                img_draw.point((j, i), fill=Cube.colorDict[self.back[i - self.order][j - self.order * 3]])

        for i in range(self.order * 2, self.order * 3):
            for j in range(self.order, self.order * 2):
                img_draw.point((j, i), fill=Cube.colorDict[self.down[i - self.order * 2][j - self.order]])

        return image

    def displayCube(self):
        plt.imshow(self.get_state_img())
        plt.show()
