...
def get_fitness(new_state):
    return np.count_nonzero(np.fromiter(map(lambda iv: iv[1]==new_state[0][(iv[0]//9)*9 + 4], enumerate(new_state[0])), dtype=bool))
...
#     env = gym.make(env)
    ...
    return get_fitness(new_state), total_frames, reward == 1
