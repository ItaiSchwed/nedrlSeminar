...
def evaluate_model(env, model, logger, max_eval=20000, max_noop=30):
    env = gym.make(env)
    ...
    for _ in range(max_eval):
        ...
        action = values[:env.action_space.n].argmax()
        # print(action.item(), end=' ')
        new_state, reward, is_done, _ = step(env, action.item())
        total_reward += reward
        if is_done:
            break
        cur_states.pop(0)
    ...
    return total_reward, total_frames, (time.time() - start)
